...
from net.ResUNet import ResUNet, UNet3D
...
#net = torch.nn.DataParallel(UNet3D()).cuda()
...
for file_index, file in enumerate(os.listdir(para.test_ct_path)):
    ...
    with torch.no_grad():
        while end_slice < ct_array.shape[0]:
            start_x = 0
            end_x = start_x + 112 - 1
            while end_x < ct_array.shape[1]:
                ct_array_crop = ct_array[start_slice: end_slice + 1, start_x:start_x:end_x + 1, start_x:end_x + 1]
                ct_tensor = torch.FloatTensor(ct_array_crop).cuda()
                ct_tensor = ct_tensor.unsqueeze(dim=0).unsqueeze(dim=0)

                outputs = net(ct_tensor)
                outputs = outputs[-1]
                # print(outputs.shape)
                count[start_slice: end_slice + 1, start_x: end_x + 1, start_x:end_x + 1] += 1
                probability_map[start_slice: end_slice + 1, start_x:end_x + 1, start_x:end_x + 1] += np.squeeze(
                ...
                start_x += stride_x
                end_x = start_x + 112 -1
            if end_x != ct_array.shape[1] - 1:
                end_x = ct_array.shape[1] - 1
                start_x = end_x - 112 + 1
                ct_array_crop = ct_array[start_slice: end_slice + 1, start_x:start_x:end_x + 1, start_x:end_x + 1]
                ct_tensor = torch.FloatTensor(ct_array_crop).cuda()
                ct_tensor = ct_tensor.unsqueeze(dim=0).unsqueeze(dim=0)
